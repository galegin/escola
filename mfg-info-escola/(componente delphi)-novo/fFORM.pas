...
type
  ...
  TcFORM = class(TForm)
    ...
    function f_VerFK(vCdCampo : String) : String;
    function f_VerLK(vCdCampo : String) : String;
    function f_VerCB(vCdCampo : String) : String;
    ...
    procedure p_GeraListaStrings(vItems : TStrings; pLstVal : String);
    procedure p_GeraListaComboBox(vComboBox : TcComboBox; pLstVal : String);
...
uses
  ...
  begin
    ...
    bAutMan := IfNullB(LerIni(cCaption, AUT_MAN), False); //LerIniB(AUT_MAN);
    bDplMan := IfNullB(LerIni(cCaption, DPL_MAN), False); //LerIniB(DPL_MAN);

    iAltura := IfNullI(LerIni(cCaption, ALT_MAN), 0);
    iLargura := IfNullI(LerIni(cCaption, LAR_MAN), 0);

    cAtaMan := IfNull(LerIni(cCaption, ATA_MAN), cAtaMan);
    cColMan := IfNull(LerIni(cCaption, COL_MAN), cColMan);
    cTamMan := IfNull(LerIni(cCaption, TAM_MAN), cTamMan);
    cDecMan := IfNull(LerIni(cCaption, DEC_MAN), cDecMan);
    cIncMan := getitem(IfNull(LerIni(cCaption, INC_MAN), cIncMan));
    cKeyMan := getitem(IfNull(LerIni(cCaption, KEY_MAN), cKeyMan));
    cValMan := getitem(IfNull(LerIni(cCaption, VAL_MAN), cValMan));
...
begin
  ...
  {
  putitemX(cCB, 'TP_SITUACAO', 'Normal|Cancelado|Excluido');
  putitemX(cFK, 'CD_GENERO', 'DS_TABELA=GER_GENERO;DS_CAMPO=DS_GENERO');
  putitemX(cTB, 'GER_LOCADOR', 'DS_CAMPO=CD_LOCADOR|NM_LOCADOR|;DS_TAM=10|40|;DS_CHAVE=CD_LOCADOR|;NR_ALTURA=05;NR_LARGURA=80');
  putitemX(cFK, 'CD_CURSO', 'DS_TABELA=GER_CURSO;CD_KEY=CD_CURSO;DS_KEY=NM_CURSO');
  putitemX(cTB, 'GER_CURSO', 'DS_CAMPO=CD_CURSO|NM_CURSO|;DS_TAM=10|40|;DS_CHAVE=CD_CURSO|;NR_ALTURA=05;NR_LARGURA=80');
  }
...
function TcFORM.f_VerFK(vCdCampo : String) : String;
begin
  Result := itemX(vCdCampo, cFK);
  if Result <> '' then putitem(Result, 'CD_CAMPO', vCdCampo);
end;

function TcFORM.f_VerLK(vCdCampo : String) : String;
begin
  Result := itemX(vCdCampo, cLK);
  if Result <> '' then putitem(Result, 'CD_CAMPO', vCdCampo);
end;

function TcFORM.f_VerCB(vCdCampo : String) : String;
begin
  Result := itemX(vCdCampo, cCB);
end;
...
var
  ...
  vCB, vLK, vFK, vVerFK, vVerLK, vVerCB : String;
...
begin
  ...
  with _DataSet do begin
    ...
    for I := 0 to FieldCount-1 do begin
      with Fields[I] do begin
        vCB := LerIni(cCaption, 'cb' + FieldName);
        if (vCB <> '') then putitemX(cCB, FieldName, vCB);
        vFK := LerIni(cCaption, 'fk' + FieldName);
        if (vFK <> '') then putitemX(cFK, FieldName, vFK);
        vLK := LerIni(cCaption, 'lk' + FieldName);
        if (vLK <> '') then putitemX(cLK, FieldName, vLK);
      end;
    end;
    ...
    for I := 0 to FieldCount-1 do begin
      with Fields[I] do begin
        ...
        if (cKeyMan <> '') then begin
          vTag := IffThenI(Pos(FieldName, cKeyMan) > 0, TAG_PK, TAG_NORMAL);
        end;
        ...
        if not vInCriado then begin
          ...
          vLabel.Caption := DisplayLabel + iff(Required, ' *', '');
          if (Required) then
          ...
          if not vInCriado then begin
            vVerLK := f_VerLK(FieldName);
            if (vVerLK = '') then begin
              vVerCB := f_VerCB(FieldName);
            end;
            ...
            if (vVerCB <> '') or (vVerLK <> '') then begin
              ...
              if (vVerLK <> '') then begin
                dDADOS.p_GeraTabelaComboBox(vComboBox, vVerLK);
              end else if (vVerCB <> '') then begin
                p_GeraListaComboBox(vComboBox, vVerCB);
              end;
          ...
          if not vInCriado then begin
            ...
            if (vVerFK <> '') then begin
              ...
              if IfNullB(LerIni(cCaption, CAD_DES), False) then begin
                vEditD.Color := clWindow;
                vEditD.ReadOnly := False;
                vEditD.TabStop := True;
                vEditD.OnExit := EditExitCad;
              end;
...
procedure TcFORM.p_GeraListaStrings(vItems : TStrings; pLstVal : String);
var
  vVal : String;
begin
  vItems.Clear;
  while pLstVal <> '' do begin
    vVal := getitem(pLstVal);
    if vVal = '' then Break;
    delitem(pLstVal);
    vItems.Add(vVal);
  end;
...
procedure TcFORM.p_GeraListaComboBox(vComboBox : TcComboBox; pLstVal : String);
var
  vVal : String;
  I : Integer;
begin
  ...
  while pLstVal <> '' do begin
    ...
    if vVal = '' then Break;
    delitem(pLstVal);
    vComboBox.p_AddRow(IntToStr(I), vVal);
    Inc(I);
  end;
end;
...
var
  vParams, vResult, vSql, vEnt, vCod, vDes, vSeq : String;
  vEditCod, vEditDes : TEdit;
begin
  vEditDes := TEdit(Sender);
  if vEditDes.Text = '' then
    Exit;

  vCod := vEditDes.Name;
  Delete(vCod, 1, 1);

  vEditCod := TEdit(FindComponent(vCod));
  if vEditCod = nil then
    Exit;
  if vEditCod.Text <> '' then
    Exit;
  ...
  vSql :=
    'select * from ' + vEnt + ' ' +
    'where ' + vDes + '=''' + vEditDes.Text + ''' ' ;

  vResult := dDADOS.f_ConsultaStrSql(vSql, '*');
  if item(vCod, vResult) <> '' then begin
    vEditCod.Text := item(vCod, vResult);
    Exit;
  end;
...
begin
  if (TEdit(Sender).Tag <> TAG_FK) then Exit;
  EditExit(Sender);
end;
...
begin
  for I := 0 to ComponentCount - 1 do begin
    if (Components[I] is TEdit) then begin
        ...
        if (Tag = TAG_FK) then begin
          if (Hint = '') then begin
            Hint := f_VerFK(Copy(Name, 2, Length(Name)-1));
          end;
          if (Hint <> '') then begin
            OnChange := EditChange;
            OnExit := EditExit;
      ...
      with Fields[I] do begin
        if (Copy(FieldName, 1, 3) = 'TP_') then begin
          if (FieldName = 'TP_SITUACAO') then begin
            ...
            if (f_VerCB(FieldName) <> '') then begin
...
begin
  if (Copy(TField(Sender).FieldName, 1, 3) = 'TP_') then begin
    vDsCampo := f_VerCB(TField(Sender).FieldName);
    Text := getitem(vDsCampo, TField(Sender).asInteger);
  end else begin
...
var
  vVerFK : String;
begin
  vVerFK := f_VerFK(TField(Sender).FieldName);
  if (vVerFK <> '') then begin
...
begin
  ...
  vTabela := IfNull(LerIni('GER_' + item('DS_TABELA', vTB), TAB_MAN), item('DS_TABELA', vTB));
...
begin
  TcLOGALTERACAO.p_Consultar(_DataSet, cTabMan);
end;
...
begin
  ...
  TcOBS.Edita_Obs(_DataSet, cTabMan);
end;

procedure TcFORM.bConfigurarManutencaoClick(Sender: TObject);
begin
  TcCONFMANUT.ConfigurarManut(cCaption, cTabMan);
end;
