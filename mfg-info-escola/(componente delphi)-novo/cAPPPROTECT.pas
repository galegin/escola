...
uses
  fAppProtect, ucFuncao, ucItem, ucXml;
...
resourceString
  ...
  var
    ...
    Unidade: String;
  begin
    Unidade := IfNullS(itemX('CD_UNIDADE', pParams), 'C');
  ...
  begin
    ...
    with TcAppProtect.Create(nil) do begin
      ...
      FDateStart := StrToDate(IfNullS(item('DT_INICIAL', pParams), cMIN_DATA));
      FDateEnd := StrToDate(IfNullS(item('DT_FINAL', pParams), cMAX_DATA));
...
begin
  ...
  FAppName := IfNullS(Application.Name, Application.Title);
  FRegKey := REGKEY_PATH;
  FOwner := AOwner;
  Randomize;
end;
...
begin
  ...
  // validade do número
  if (Result = False) then ShowMessage(SCodErro);
end;
