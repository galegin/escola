...
begin
  ...
  try
    ...
    RaiseException(ExceptionType, 'Please install Microsoft IIS first.'#13#13'(Error '''+ExceptionParam+''' occured)');
  end;
  ...
  if not InstShellExec(IISURL + '/innosetup/index.htm', '', '', SW_SHOWNORMAL, ErrorCode) then
...
begin
  ...
  try
    ...
    RaiseException(ExceptionType, 'Please install MSXML first.'#13#13'(Error '''+ExceptionParam+''' occured)');
  end;
  ...
  if XMLDoc.parseError.errorCode <> 0 then
    RaiseException(erCustomError, 'Error on line ' + IntToStr(XMLDoc.parseError.line) + ', position ' + IntToStr(XMLDoc.parseError.linepos) + ': ' + XMLDoc.parseError.reason);
